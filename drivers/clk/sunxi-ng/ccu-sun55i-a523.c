...
/* register dump suggests PLL at +0x080, potentially AUDIO1? */
...
static SUNXI_CCU_M_DATA_WITH_MUX(ahb_clk, "ahb", ahb_parents, 0x510,
				 0, 5,		/* M */
				 24, 2,		/* mux */
				 0);
...
static SUNXI_CCU_M_DATA_WITH_MUX(apb0_clk, "apb0", apb0_parents, 0x520,
				 0, 5,		/* M */
				 24, 2,	/* mux */
				 0);
...
static const struct clk_hw *de_parents[] = {
	&pll_periph0_300M_clk.common.hw,
	&pll_periph0_400M_clk.common.hw,
	&pll_video3_4x_clk.common.hw,
	&pll_video3_3x_clk.common.hw,
};
...
static SUNXI_CCU_GATE_HWS(bus_di_clk, "bus-di", ahb_hws,
			  0x62c, BIT(0), 0);
...
static const struct clk_hw *gpu_parents[] = {
	...
	&pll_periph0_600M_clk.common.hw,
	&pll_periph0_400M_clk.common.hw,
	&pll_periph0_300M_clk.common.hw,
	&pll_periph0_200M_clk.common.hw,
};
...
static SUNXI_CCU_GATE_HWS(bus_gpu_clk, "bus-gpu", ahb_hws,
			  0x67c, BIT(0), 0);

static const struct clk_parent_data ce_parents[] = {
	{ .fw_name = "hosc" },
	{ .hw = &pll_periph0_480M_clk.common.hw },
	{ .hw = &pll_periph0_400M_clk.common.hw },
	{ .hw = &pll_periph0_300M_clk.common.hw },
};
...
static SUNXI_CCU_GATE_HWS(bus_ce_clk, "bus-ce", ahb_hws,
			  0x68c, BIT(0), 0);
static SUNXI_CCU_GATE_HWS(bus_ce_sys_clk, "bus-ce-sys", ahb_hws,
			  0x68c, BIT(1), 0);
...
static const struct clk_parent_data hstimer_parents[] = {
	...
	{ .fw_name = "iosc" },
	{ .fw_name = "losc" },
	{ .hw = &pll_periph0_200M_clk.common.hw },
};
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer0_clk, "hstimer0", hstimer_parents, 0x730,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
...
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer1_clk, "hstimer1", hstimer_parents, 0x734,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
...
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer2_clk, "hstimer2", hstimer_parents, 0x738,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
...
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer3_clk, "hstimer3", hstimer_parents, 0x73c,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
...
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer4_clk, "hstimer4", hstimer_parents, 0x740,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
				     CLK_SET_RATE_PARENT);
static SUNXI_CCU_MP_HW_WITH_MUX_GATE(hstimer5_clk, "hstimer5", hstimer_parents, 0x744,
				     0, 0,	/* M */
				     0, 3,	/* P */
				     24, 3,	/* mux */
				     BIT(31),	/* gate */
...
static const struct clk_parent_data iommu_parents[] = {
	{ .hw = &pll_periph0_600M_clk.common.hw },
	{ .hw = &pll_ddr0_clk.common.hw },
	{ .hw = &pll_periph0_480M_clk.common.hw },
	{ .hw = &pll_periph0_400M_clk.common.hw },
	{ .hw = &pll_periph0_150M_clk.common.hw },
	{ .fw_name = "hosc" },
};

static SUNXI_CCU_M_HW_WITH_MUX_GATE(iommu_clk, "iommu", iommu_parents, 0x7b0,
				    0, 5,	/* M */
				    24, 3,	/* mux */
				    BIT(31),	/* gate */
				    CLK_SET_RATE_PARENT);
...
static const struct clk_hw *dram_parents[] = {
	...
	&pll_periph0_480M_clk.common.hw,
	&pll_periph0_400M_clk.common.hw,
	&pll_periph0_150M_clk.common.hw,
};
...
static const struct clk_parent_data nand_mmc_parents[] = {
	...
	{ .hw = &pll_periph0_400M_clk.common.hw },
	{ .hw = &pll_periph0_300M_clk.common.hw },
	{ .hw = &pll_periph1_400M_clk.common.hw },
	{ .hw = &pll_periph1_300M_clk.common.hw },
};

static SUNXI_CCU_M_HW_WITH_MUX_GATE(nand0_clk, "nand0", nand_mmc_parents, 0x810,
				    0, 5,	/* M */
				    24, 3,	/* mux */
				    BIT(31),	/* gate */
				    0);

static SUNXI_CCU_M_HW_WITH_MUX_GATE(nand1_clk, "nand1", nand_mmc_parents, 0x814,
				    0, 5,	/* M */
				    24, 3,	/* mux */
				    BIT(31),	/* gate */
				    0);

static SUNXI_CCU_GATE_HWS(bus_nand_clk, "bus-nand", psi_ahb_hws,
			  0x82c, BIT(0), 0);

static SUNXI_CCU_MP_DATA_WITH_MUX_GATE(mmc0_clk, "mmc0", nand_mmc_parents, 0x830,
				       0, 5,	/* M */
				       8, 5,	/* P */
				       24, 3,	/* mux */
				       BIT(31),	/* gate */
...
static const struct clk_parent_data mmc2_parents[] = {
	...
	{ .hw = &pll_periph1_600M_clk.common.hw },
};
...
static SUNXI_CCU_GATE_HWS(bus_uart5_clk, "bus-uart5", apb1_hws,
			  0x90c, BIT(5), 0);
static SUNXI_CCU_GATE_HWS(bus_uart6_clk, "bus-uart6", apb1_hws,
			  0x90c, BIT(6), 0);
static SUNXI_CCU_GATE_HWS(bus_uart7_clk, "bus-uart7", apb1_hws,
			  0x90c, BIT(7), 0);
...
static SUNXI_CCU_GATE_HWS(bus_i2c4_clk, "bus-i2c4", apb1_hws,
			  0x91c, BIT(4), 0);
static SUNXI_CCU_GATE_HWS(bus_i2c5_clk, "bus-i2c5", apb1_hws,
			  0x91c, BIT(5), 0);
...
static const struct clk_parent_data spi_parents[] = {
	...
	{ .hw = &pll_periph0_200M_clk.common.hw },
	{ .hw = &pll_periph1_300M_clk.common.hw },
	{ .hw = &pll_periph1_200M_clk.common.hw },
};
...
static SUNXI_CCU_MP_DATA_WITH_MUX_GATE(spifc_clk, "spifc", nand_mmc_parents, 0x950,
				       0, 5,	/* M */
				       8, 5,	/* P */
				       24, 3,	/* mux */
				       BIT(31),	/* gate */
...
static SUNXI_CCU_GATE_HWS(bus_spi1_clk, "bus-spi1", psi_ahb_hws,
			  0x96c, BIT(1), 0);
static SUNXI_CCU_GATE_HWS(bus_spi2_clk, "bus-spi2", psi_ahb_hws,
			  0x96c, BIT(2), 0);
static SUNXI_CCU_GATE_HWS(bus_spifc_clk, "bus-spifc", psi_ahb_hws,
			  0x96c, BIT(3), 0);

/*TODO: Actually 150MHz Periph0 / 6 */
//	{ .hw = &pll_periph0_150M_clk.common.hw },
...
static SUNXI_CCU_M_HW_WITH_MUX_GATE(ir_rx_clk, "ir-rx", ir_rx_parents, 0x990,
				    0, 5,	/* M */
				    24, 1,	/* mux */
				    BIT(31),	/* gate */
				    0);

static SUNXI_CCU_GATE_HWS(bus_ir_rx_clk, "bus-ir-rx", apb0_hws,
			  0x99c, BIT(0), 0);
...
static SUNXI_CCU_M_DATA_WITH_MUX_GATE(ir_tx_clk, "ir-tx", ir_tx_ledc_parents, 0x9c0,
				       0, 5,	/* M */
				       24, 1,	/* mux */
				       BIT(31),	/* gate */
				       0);
...
static SUNXI_CCU_M_HWS_WITH_GATE(gpadc_clk, "gpadc", "hosc", 0x9e0,
				0, 5,		/* M */
				BIT(31),	/* gate */
				0);
...
static SUNXI_CCU_GATE_DATA_WITH_PREDIV(fanout_12M_clk, "fanout-12M", osc24M,
				       0xf30, BIT(1), 2, 0);
static SUNXI_CCU_GATE_HWS_WITH_PREDIV(fanout_16M_clk, "fanout-16M", pll_periph0_480M_hws,
				      0xf30, BIT(2), 30, 0);
static SUNXI_CCU_GATE_HWS_WITH_PREDIV(fanout_25M_clk, "fanout-25M", pll_periph0_2x_hws,
				      0xf30, BIT(3), 48, 0);
static SUNXI_CCU_GATE_HWS_WITH_PREDIV(fanout_50M_clk, "fanout-50M", pll_periph0_2x_hws,
				      0xf30, BIT(4), 24, 0);
...
static const struct clk_hw *fanout_27M_parents[] = {
	&pll_video0_4x_clk.hw,
	&pll_video1_4x_clk.hw,
	&pll_video2_4x_clk.hw,
	&pll_video3_4x_clk.hw,
};
...
static const struct clk_hw *fanout_parents[] = {
	&fanout_32k_clk.common.hw,	/* TODO: from SYSRTC? */
